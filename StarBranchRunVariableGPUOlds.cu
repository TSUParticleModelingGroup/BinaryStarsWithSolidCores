/*
nvcc StarBranchRunVariableGPUs.cu -o StarBranchRunVariableGPUs.exe -lglut -lGL -lGLU -lm
nvcc StarBranchRunVariableGPUs.cu -o StarBranchRunVariableGPUs.exe -lglut -lGL -lGLU -lm --use_fast_math
*/
...
__global__ void getForces(float4 *pos, float4 *vel, float4 *force, int numberElementsStar1, int numberOfElements, float growthMultiplier1, float growthMultiplier2, float corePushBackReduction, float plasmaPushBackReduction, int gPUNumber, int gPUsUsed)
{
	int id, ids, i, j, k;
	...
	id = threadIdx.x + blockDim.x*blockIdx.x + gridDim.x*gPUNumber;
...
{  
    	int id = threadIdx.x + blockDim.x*blockIdx.x + gridDim.x*gPUNumber;
...
{ 
	...
	{	
		...
		for(int i = 0; i < gPUsUsed; i++)
		{
			cudaSetDevice(i);
			errorCheck("cudaSetDevice");
			getForces<<<GridConfig, BlockConfig>>>(PosGPU[i], VelGPU[i], ForceGPU[i], NumberElementsStar1, NumberElements, growthMultiplier1, growthMultiplier2, CorePushBackReduction, PlasmaPushBackReduction, i, gPUsUsed);
		...
		for(int i = 0; i < gPUsUsed; i++)
		{
			...
			for(int j = 0; j < gPUsUsed; j++)
			{
				if(i != j)
				{
					cudaMemcpyPeerAsync(PosGPU[i] + i*offSet,i,PosGPU[j] + i*offSet,j,(NumberElements/gPUsUsed)*sizeof(float4));
					errorCheck("cudaMemcpyPeerAsync Pos");
					cudaMemcpyPeerAsync(VelGPU[i] + i*offSet,i,VelGPU[j] + i*offSet,j,(NumberElements/gPUsUsed)*sizeof(float4));
					errorCheck("cudaMemcpyPeerAsync Vel");
				}
			}
		}
		cudaDeviceSynchronize();
