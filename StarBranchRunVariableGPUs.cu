...
float4 *PosGPU_0, *VelGPU_0, *ForceGPU_0;

//Global to hold the number of GPUs
int GPUsUsed
...
void deviceSetup()
{
	...
	else if(numberOfGpus == 2 || numberOfGpus == 3) //We are only using even numbers of GPUs so 3 will be down graded to 2
	{
		...
		if(access == 0)
		{
			printf("\nTSU Error: Device0 can not do peer to peer. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		...
		if(access == 0)
		{
			printf("\nTSU Error: Device1 can not do peer to peer. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		...
		if(ok == 1)
		{
			...
			GridConfig.x = ((NumberElements/2)-1)/BlockConfig.x + 1;
			GridConfig.y = 1;
			GridConfig.z = 1;
			
			cudaSetDevice(0);
			cudaMalloc((void**)&PosGPU_0, NumberElements *sizeof(float4));
			errorCheck("cudaMalloc Pos_0");
			cudaMalloc((void**)&VelGPU_0, NumberElements *sizeof(float4));
			errorCheck("cudaMalloc Vel_0");
			cudaMalloc((void**)&ForceGPU_0, NumberElements *sizeof(float4));
			errorCheck("cudaMalloc Force_0");
			
			cudaSetDevice(1);
			cudaMalloc((void**)&PosGPU_1, NumberElements *sizeof(float4));
			errorCheck("cudaMalloc Pos_1");
			cudaMalloc((void**)&VelGPU_1, NumberElements *sizeof(float4));
			errorCheck("cudaMalloc Vel_1");
			cudaMalloc((void**)&ForceGPU_1, NumberElements *sizeof(float4));
			errorCheck("cudaMalloc Force_1");
		}
	}
	else
	{
		...
		if(access == 0)
		{
			printf("\nTSU Error: Device0 can not do peer to peer to device1. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,0,2);
		errorCheck("cudaDeviceCanAccessPeer1");
		if(access == 0)
		{
			printf("\nTSU Error: Device0 can not do peer to peer to device2. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,0,3);
		errorCheck("cudaDeviceCanAccessPeer1");
		if(access == 0)
		{
			printf("\nTSU Error: Device0 can not do peer to peer to device3. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		...
		if(access == 0)
		{
			printf("\nTSU Error: Device1 can not do peer to peer to device0. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,1,2);
		errorCheck("cudaDeviceCanAccessPeer1");
		if(access == 0)
		{
			printf("\nTSU Error: Device1 can not do peer to peer to device2. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,1,3);
		errorCheck("cudaDeviceCanAccessPeer1");
		if(access == 0)
		{
			printf("\nTSU Error: Device1 can not do peer to peer to device3. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,2,0);
		errorCheck("cudaDeviceCanAccessPeer0");
		if(access == 0)
		{
			printf("\nTSU Error: Device2 can not do peer to peer to device0. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,2,1);
		errorCheck("cudaDeviceCanAccessPeer1");
		if(access == 0)
		{
			printf("\nTSU Error: Device2 can not do peer to peer to device1. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,2,3);
		errorCheck("cudaDeviceCanAccessPeer1");
		if(access == 0)
		{
			printf("\nTSU Error: Device2 can not do peer to peer to device3. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,3,0);
		errorCheck("cudaDeviceCanAccessPeer0");
		if(access == 0)
		{
			printf("\nTSU Error: Device3 can not do peer to peer to device0. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,3,1);
		errorCheck("cudaDeviceCanAccessPeer1");
		if(access == 0)
		{
			printf("\nTSU Error: Device3 can not do peer to peer to device1. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		
		cudaDeviceCanAccessPeer(&access,3,2);
		errorCheck("cudaDeviceCanAccessPeer1");
		if(access == 0)
		{
			printf("\nTSU Error: Device3 can not do peer to peer to device2. Dropping to using only 1 GPU\n");
			GPUsUsed = 1;
			ok = 0;
		}
		...
		if(ok == 1)
		{
			...
			GridConfig.x = ((NumberElements/4)-1)/BlockConfig.x + 1;
			GridConfig.y = 1;
			GridConfig.z = 1;
			
			cudaSetDevice(0);
				cudaDeviceEnablePeerAccess(1,0);
				errorCheck("cudaDeviceEnablePeerAccess 0 to 1");
				cudaDeviceEnablePeerAccess(2,0);
				errorCheck("cudaDeviceEnablePeerAccess 0 to 2");
				cudaDeviceEnablePeerAccess(3,0);
				errorCheck("cudaDeviceEnablePeerAccess 0 to 3");
			...
			cudaSetDevice(1);
				...
				errorCheck("cudaDeviceEnablePeerAccess 1 to 0");
				cudaDeviceEnablePeerAccess(2,0);
				errorCheck("cudaDeviceEnablePeerAccess 1 to 2");
				cudaDeviceEnablePeerAccess(3,0);
				errorCheck("cudaDeviceEnablePeerAccess 1 to 3");
			...
			cudaSetDevice(2);
				cudaDeviceEnablePeerAccess(0,0);
				errorCheck("cudaDeviceEnablePeerAccess 2 to 0");
				cudaDeviceEnablePeerAccess(1,0);
				errorCheck("cudaDeviceEnablePeerAccess 2 to 1");
				cudaDeviceEnablePeerAccess(3,0);
				errorCheck("cudaDeviceEnablePeerAccess 2 to 3");
				
				cudaMalloc((void**)&PosGPU_2, NumberElements *sizeof(float4));
				errorCheck("cudaMalloc Pos_2");
				cudaMalloc((void**)&VelGPU_2, NumberElements *sizeof(float4));
				errorCheck("cudaMalloc Vel_2");
				cudaMalloc((void**)&ForceGPU_2, NumberElements *sizeof(float4));
				errorCheck("cudaMalloc Force_2");
				
				
			
			cudaSetDevice(3);
				cudaDeviceEnablePeerAccess(0,0);
				errorCheck("cudaDeviceEnablePeerAccess 3 to 0");
				cudaDeviceEnablePeerAccess(1,0);
				errorCheck("cudaDeviceEnablePeerAccess 3 to 1");
				cudaDeviceEnablePeerAccess(2,0);
				errorCheck("cudaDeviceEnablePeerAccess 3 to 2");
				
				cudaMalloc((void**)&PosGPU_3, NumberElements *sizeof(float4));
				errorCheck("cudaMalloc Pos_3");
				cudaMalloc((void**)&VelGPU_3, NumberElements *sizeof(float4));
				errorCheck("cudaMalloc Vel_3");
				cudaMalloc((void**)&ForceGPU_3, NumberElements *sizeof(float4));
				errorCheck("cudaMalloc Force_3");
		}
	}
...
__global__ void getForces(float4 *pos, float4 *vel, float4 *force, int numberElementsStar1, int numberOfElements, float growthMultiplier1, float growthMultiplier2, float corePushBackReduction, float plasmaPushBackReduction)
{
	...
	for(k =0; k < numberOfGPUsUsed; k++)
	{
		for(j = 0; j < gridDim.x; j++)
		{
			shPos[threadIdx.x]   = pos  [threadIdx.x + blockDim.x*j + gridDim.x*k];
			shVel[threadIdx.x]   = vel  [threadIdx.x + blockDim.x*j + gridDim.x*k];
			shForce[threadIdx.x] = force[threadIdx.x + blockDim.x*j + gridDim.x*k];
			__syncthreads();
		   
			#pragma unroll 32
			for(i = 0; i < blockDim.x; i++)	
			{
				ids = i + blockDim.x*j + gridDim.x*k;
				if(id != ids)
				{
...
{ 
	...
	{	
		if(GPUsUsed == 1)
		{
			...
			getForces<<<GridConfig, BlockConfig>>>(PosGPU, VelGPU, ForceGPU, NumberElementsStar1, NumberElements, growthMultiplier1, growthMultiplier2, CorePushBackReduction, PlasmaPushBackReduction, elementStart, elementStop);
			errorCheck("getForcesSeperate");
			
			moveBodies<<<GridConfig, BlockConfig>>>(PosGPU, VelGPU, ForceGPU, dt, elementStart, elementStop);
			errorCheck("moveBodiesDamped");
			
			//No need to share memory because youare only using 1 GPU.
		}
		else if(GPUsUsed == 2)
		{
			elementStart = 0;
			elementStop = NumberElements/2;
			getForces<<<GridConfig, BlockConfig>>>(PosGPU, VelGPU, ForceGPU, NumberElementsStar1, NumberElements, growthMultiplier1, growthMultiplier2, CorePushBackReduction, PlasmaPushBackReduction, elementStart, elementStop);
			errorCheck("getForcesSeperate");
			...
			shareMemoryBetweenGPUs() ??????????????????????
		}
		else if(GPUsUsed == 4)
		{
			elementStart = 0;
			elementStop = NumberElements/4;
			getForces<<<GridConfig, BlockConfig>>>(PosGPU, VelGPU, ForceGPU, NumberElementsStar1, NumberElements, growthMultiplier1, growthMultiplier2, CorePushBackReduction, PlasmaPushBackReduction, elementStart, elementStop);
			errorCheck("getForcesSeperate");
			...
			elementStart = NumberElements/2;
			elementStop = NumberElements*3/4;
			getForces<<<GridConfig, BlockConfig>>>(PosGPU, VelGPU, ForceGPU, NumberElementsStar1, NumberElements, growthMultiplier1, growthMultiplier2, CorePushBackReduction, PlasmaPushBackReduction, elementStart, elementStop);
			errorCheck("getForcesSeperate");
			...
			elementStart = NumberElements*3/4;
			elementStop = NumberElements;
			getForces<<<GridConfig, BlockConfig>>>(PosGPU, VelGPU, ForceGPU, NumberElementsStar1, NumberElements, growthMultiplier1, growthMultiplier2, CorePushBackReduction, PlasmaPushBackReduction, elementStart, elementStop);
			errorCheck("getForcesSeperate");
