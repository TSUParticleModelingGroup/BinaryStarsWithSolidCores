...
void deviceSetup()
{
	...
	get propertise and set the number of GPUs ?????????????????????????????????
}
...
void shareMemoryBetweenGPUs()
{
	...
	else if(NumberGpus == 1)
	{
	
	}
	else if(NumberGpus == 2 || NumberGpus == 3) //We are only using even numbers of GPUs so 3 will be down graded to 2
	{
		if(NumberGpus == 3) 
		{
