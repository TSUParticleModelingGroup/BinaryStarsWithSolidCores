	...
	{	
		...
		for(int i = 0; i < gPUsUsed; i++)
		{
			...
			for(int j = 0; j < gPUsUsed; j++)
			{
				if(i != j)
				{
					cudaMemcpy(PosGPU[j] + i*offSet, PosGPU[i] + i*offSet, (NumberElements/gPUsUsed)*sizeof(float4), cudaMemcpyDeviceToDevice);
					errorCheck("cudaMemcpy Pos");
					
					cudaMemcpy(VelGPU[j] + i*offSet, VelGPU[i] + i*offSet, (NumberElements/gPUsUsed)*sizeof(float4), cudaMemcpyDeviceToDevice);
